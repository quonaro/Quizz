...
class AnswerChecker:
    ...
    def check_multiple_choice(
        user_answer: List[int], correct_answers: List[int], partial_credit: bool = False
    ) -> tuple[bool, float]:
        """
    ...
    @staticmethod
    def check_matching(
        user_answer: Dict[int, int], correct_matches: List[Dict[str, int]]
    ) -> tuple[bool, float]:
        """
    ...
    @staticmethod
    def check_question(
        question_data: Dict[str, Any], user_answer: Any
        ...
        if q_type == "single_choice":
            correct = AnswerChecker.check_single_choice(
                user_answer, question_data.get("correct_answer", -1)
            )
            ...
            correct, score_ratio = AnswerChecker.check_matching(
                user_answer or {}, question_data.get("matches", [])
            )
            if correct:
                feedback = "Правильно!"
            elif score_ratio > 0:
            ...
            correct = AnswerChecker.check_ordering(
                user_answer or [], question_data.get("correct_order", [])
            )
            score = 1.0 if correct else 0.0
            feedback = "Правильно!" if correct else "Неправильно"
            return correct, score, feedback
    ...
    @staticmethod
    def check_quiz(
        quiz_data: Dict[str, Any], user_answers: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for question in questions:
            ...
            is_correct, score_ratio, feedback = AnswerChecker.check_question(
            ...
            results["questions"].append(
                {
                    ...
                    "correct_answer": AnswerChecker._get_correct_answer_display(
                        question
                    ),
    ...
    def _get_correct_answer_display(question_data: Dict[str, Any]) -> Any:
        ...
        if q_type == "single_choice":
            ...
            for m in matches:
                ...
                if 0 <= left_idx < len(left_items) and 0 <= right_idx < len(
                    right_items
                ):
